    ...
    class UPCP_Product {
    	...
    	function Get_Product_Price($Return_Type = "Int", $Sale = "Implied") {
    		...
    		if ($Sale == "Sale" or ($Sale == "Implied" and ($Sale_Mode == "All" or ($Item_Sale == "Yes" and $Sale_Mode == "Individual")))) {
    			$Price = $Item_Link = $wpdb->get_var($wpdb->prepare("SELECT Item_Sale_Price FROM $items_table_name WHERE Item_ID=%d", $this->Item_ID));
			...
			$Item_Full_Price = $this->Get_Product_Price("Currency", "Regular");
			
			if ($Price != $Item_Full_Price) {
				$Item_Display_Price = "<span class='price'>";
				$Item_Display_Price .= "<del><span class='amount'>" . $Item_Full_Price . "</span></del>";
				$Item_Display_Price .= "<ins><span class='amount'>" . $Price . "</span></ins>";
				$Item_Display_Price .= "</span>"; 
			}
			else {$Item_Display_Price = $Price;}
